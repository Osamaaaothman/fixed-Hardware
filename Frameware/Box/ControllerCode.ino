...
public:
 ...
 void showModeMenu(int page = 1) {
  ...
  if (page == 1) {
    tft.setCursor(40, 100); tft.println("1: Writing");
    tft.setCursor(40, 130); tft.println("2: Erasing");
    tft.setCursor(40, 200); tft.println("A: Next Page");
  }
  else if (page == 2) {
    tft.setCursor(40, 100); tft.println("3: Exit");
    tft.setCursor(40, 200); tft.println("B: Prev Page");
  }
...
public:
  ModeHandler(DisplayManager& d, KeypadManager& k) : display(d), keypad(k) {}

  void writingMode() {
    display.clear();
    display.drawTitle("Writing Mode");
    display.tft.setTextColor(ST77XX_WHITE);
    display.tft.setTextSize(2);
    display.centerText("Press # to exit", 280);
    bool inProcessing = true;
    int y = 160;
    while (inProcessing) {
      display.rgbEffect("writing", 15);
      for (int x = 20; x < 220 && inProcessing; x += 10) {
        char key = keypad.getKey();
        if (key == '#') { inProcessing = false; break; }
    ...
    display.clear();
    display.drawTitle("Done Writing");
  ...
  void erasingMode() {
    ...
    while (inProcessing) {
      ...
      for (int x = 0; x < 240 && inProcessing; x += 20) {
        char key = keypad.getKey();
        if (key == '#') { inProcessing = false; break; }
        display.tft.fillRect(x, 100, 20, 80, ST77XX_RED);
        delay(60);
        display.tft.fillRect(x, 100, 20, 80, ST77XX_BLACK);
...
void handleServerCommand(String cmd) {
    ...
    if (cmd == "writing") {
        Serial.println("[Server] → Writing Mode triggered");
        modes.writingMode();
        ...
        Serial.println("[Server] → Erasing Mode triggered");
        modes.erasingMode();
        display.showModeMenu();
  ...
  void tick() {
    ...
    if (security.isLocked()) {
      ...
      if (rem == 0) {
        ...
        loginStage = WAIT_RFID;
        display.clear();
        display.drawTitle("Unlocked");
        display.requestRFIDAnimation();
        Serial.println("[System] unlocked after lock duration");
      }
      return;
    }
  ...
  void handleLogin() {
    ...
    if (systemActive) {
      ...
      if (now - lastActivityTime >= timeoutMs) {
        Serial.println("[System] inactivity timeout -> enterSleep()");
        enterSleep();
